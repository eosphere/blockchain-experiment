...
void experiment::setexchange ( const symbol& symbol, const double& rate ) {
   require_auth (get_self());

   config_table      config_s (get_self(), get_self().value);
   config c = config_s.get_or_create (get_self(), config());

   c.token_exchange[symbol] = rate;
   config_s.set(c, get_self());
}
...
void experiment::claim( const uint64_t& serial_no, const symbol& div_symbol ) {
   ...
   if (div_symbol != c.deposit_symbol) {
      check (c.token_exchange.find(div_symbol) != c.token_exchange.end(), "Symbol not supported!");
      double exchange_rate = c.token_exchange[div_symbol];
      prize = asset (prize.amount * exchange_rate, div_symbol);
   }
